...
MCP 服务内置两层安全机制：**认证**（验证身份）和**授权**（控制访问范围），通过 `.env` 文件配置。

### 认证模式

`MCP_AUTH_MODE` 决定认证方式：

| 模式 | 说明 | 适用场景 |
|------|------|---------|
| `api_key` | 令牌验证模式（默认），由 `MCP_JWT_MODE` 决定具体验证方式 | 灵活，支持简单密钥到 JWT 验证 |
| `oauth` | OAuth 2.1 授权码流程（需配置 `MCP_OAUTH_USERS`） | 生产环境，完整 OAuth 支持 |
...
| `MCP_AUTH_API_KEY` | API Key 模式的简单密钥 | 空 |
| `MCP_SERVER_BASE_URL` | 服务基础 URL | `http://localhost:8082` |
...
| `MCP_JWT_MODE` | JWT 验证模式：`api_key`, `jwks`, `public_key`, `hmac` | `api_key` |
| `MCP_JWKS_URI` | JWKS 端点 URL（`jwks` 模式） | 空 |
| `MCP_JWT_ISSUER` | 预期 JWT 签发者（issuer） | 空 |
| `MCP_JWT_AUDIENCE` | 预期 JWT 受众（audience） | 空 |
| `MCP_JWT_ALGORITHM` | JWT 签名算法 | `RS256` |
| `MCP_JWT_PUBLIC_KEY` | PEM 格式公钥（`public_key` 模式） | 空 |
| `MCP_JWT_SECRET` | HMAC 共享密钥（`hmac` 模式） | 空 |

### API Key 模式

`MCP_AUTH_MODE=api_key` 时，`MCP_JWT_MODE` 决定具体的令牌验证方式，支持四种模式：

#### 1. 简单密钥比对（`api_key`）

默认模式，直接比对请求头中的 `Authorization: Bearer <key>` 与配置的密钥，适合开发测试。

```env
MCP_AUTH_ENABLED=true
MCP_AUTH_MODE=api_key
MCP_JWT_MODE=api_key
...
自动从 JWKS 端点获取公钥验证，支持自动密钥轮换，适合对接外部认证中心。
...
使用固定的 RSA/ECDSA 公钥验证，适合开发环境或没有 JWKS 端点的受控部署。

```env
MCP_AUTH_ENABLED=true
MCP_JWT_MODE=public_key
...
使用共享密钥（HS256/HS384/HS512）验证，适合内部微服务间通信。
...
生成带 scope 的 JWT 令牌示例：
```python
import jwt, time
token = jwt.encode({
    "sub": "user-001",
    "iss": "http://localhost:8082",
    "scope": "data forecast warning",   # 空格分隔的 scope 列表
    "exp": int(time.time()) + 3600,
}, "your-shared-secret-key", algorithm="HS256")
```
...
所有 76 个工具均通过 `require_scopes()` 设置了访问权限，令牌的 scope 决定可见的工具范围。

#### Scope 分类

| Scope | 可见工具数 | 来源文件 | 说明 |
|-------|-----------|---------|------|
| `data` | 29 | `data_api_tools.py` | 数据查询（默认最低权限） |
| `dispatch` | 9 | `reservoir_dispatch.py` | 水库调度 |
| `forecast` | 5 | `forecast_models.py` | 预报模型 |
| `plan` | 6 | `plan_tools.py` | 调度预案 |
| `simulation` | 7 | `simulation_tools.py` | 预演仿真 |
| `skill` | 1 | `skill_tools.py` | Skill 读取 |
| `ui` | 16 | `ui_tools.py` | 前端导航 |
| `warning` | 3 | `warning_tools.py` | 预警等级 |
| **全部** | **76** | — | 所有工具 |

#### 授权规则

- 令牌 scope 使用 **AND 逻辑**：令牌必须包含工具所需的所有 scope 才能访问
- 默认最低权限为 `data`（API Key 模式、OAuth 默认用户）
- 无令牌或空 scope → 0 个工具可见
...
```python
# 只可见 data 工具（29 个）
AccessToken(scopes=["data"])

# 可见 data + forecast + warning 工具（37 个）
AccessToken(scopes=["data", "forecast", "warning"])

# 可见全部 76 个工具
AccessToken(scopes=["data", "dispatch", "forecast", "plan",
                      "simulation", "skill", "ui", "warning"])
```

#### JWT 中传递 scope

JWT 令牌的 `scope` 或 `scp` 声明携带 scope 信息，支持两种格式：

```json
{
  "sub": "user-001",
  "scope": "data forecast warning",
  "exp": 1718000000
}
```
...
```bash
uv run python test/test_auth_scopes.py
```

脚本会模拟不同 scope 组合的令牌，验证工具列表是否正确过滤，输出示例：

```
📦 总工具数: 76 (预期 76)
🔴 无 token → 可见 0 个
🟡 scope=[data] → 可见 29 个
🟡 scope=[forecast] → 可见 5 个
🟢 全部 scope → 可见 76 个
```
