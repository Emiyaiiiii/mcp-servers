...
class SceneConnector:
    ...
    async def send_evacuation_routes_async(
        self,
        routes: list,
        target_session: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        Args:
            routes: 转移路线列表，每项为一个 dict：
                {
                    "id": "village_1",           // 唯一ID
                    "name": "北窑村",             // 村庄名称
                    "reservoir": "故县水库",      // 水库名称
                    "evacuation_location": "党群服务中心",  // 安置地点
                    "evacuation_route": "通村公路",  // 转移路线描述
                    "contact_name": "宋洛生",     // 联系人（可选）
                    "contact_phone": "18625797490", // 联系电话（可选）
                }
            target_session: 目标 session_id，如果为 None 则广播到所有连接
        """
        command = {
            "cmd": "lshh",
            "function": "FUNC_EVACUATION_ROUTE_SHOW",
            "target": "scene",
            "data": {
                "routes": routes,
                "action": "show"
    ...
    async def clear_evacuation_routes_async(
        self,
        route_ids: list = None,
        target_session: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        Args:
            route_ids: 要清除的路线ID列表，None表示清除全部
            target_session: 目标 session_id，如果为 None 则广播到所有连接
        """
        command = {
            "cmd": "lshh",
