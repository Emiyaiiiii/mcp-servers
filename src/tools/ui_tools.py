...
def register_ui_tools(mcp: FastMCP):
    ...
    async def highlight_warnings(
        ...
        Returns:
            发送高亮指令的确认信息
        """
        logger.info(f"调用 highlight_warnings，收到参数: markers={len(markers)} 个")
        data = {
            "markers": markers,
            "action": "show"
        }
        result = await command_sender.send_ui_command("FUNC_WARNING_HIGHLIGHT", data)
    ...
    async def clear_warning_highlights(
        ...
        Returns:
            发送清除指令的确认信息
        """
        logger.info(f"调用 clear_warning_highlights，收到参数: marker_ids={repr(marker_ids)}")
        data = {
            "action": "clear",
