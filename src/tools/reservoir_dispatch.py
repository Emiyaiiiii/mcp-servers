...
from src.utils.dispatch.dispatch_scheme import _check_dispatch_data_source, _run_regualdispacth, _process_calculation_results, _save_and_format_scheme
from src.utils.dispatch.mdb_utils import get_mdb_connection, get_dispatch_mdb_path, mdb_execute, mdb_update_field
from src.utils.model.template_utils import scan_templates, find_template_file, get_template_sheets
...
def register_reservoir_dispatch(mcp: FastMCP):
    ...
    async def generate_dispatch_scheme(
        ...
        try:
            ...
            check = _check_dispatch_data_source(exe_path, mdb_path)
            if not check["success"]:
                check["steps"] = steps
                return check
            import_elapsed = round(time.time() - import_start, 2)
            check["data_check"]["elapsed_seconds"] = import_elapsed
            steps["data_check"] = check["data_check"]
            logger.info(f"数据源检查完成: Q_Inputsd={check['sd_count']}行, Q_Inputxd={check['xd_count']}行, 耗时{import_elapsed}秒")

            # 步骤2: 运行计算
            logger.info("步骤2: 运行 RegualDispacth.exe")
            calc = await _run_regualdispacth(exe_path, dispatch_dir, steps)
            if not calc["success"]:
                return {"success": False, "error": calc["error"], "steps": steps}

            # 步骤3: 统计处理
            result = _process_calculation_results(mdb_path, steps, flood_type)
            if not result["success"]:
                return {"success": False, "error": result["error"], "steps": steps}
            result["flood_type"] = flood_type

            # 步骤4: 存储并返回
            return_value = _save_and_format_scheme(steps, result)
            return_value["total_elapsed_seconds"] = round(time.time() - total_start, 2)
            return return_value
