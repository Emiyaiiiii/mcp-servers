...
class PeerStream extends HTMLVideoElement {
  ...
  onDataChannelMessage(data) {
    ...
    switch (data[0]) {
      ...
      case RECEIVE.Response: {
        this.onReceiveMessage(JSON.parse(utf16.decode(data.slice(1))));
        break;
      }
          ...
          if (command.showOnScreenKeyboard) {
            if (this.enableChinese) {
              ...
              input.addEventListener("compositionend", (e) => {
                console.log(e.data);
                this.sendMessage(e.data, SEND.CompositionEnd);
              });
  ...
  sendMessage(msg, messageType = SEND.UIInteraction) {
